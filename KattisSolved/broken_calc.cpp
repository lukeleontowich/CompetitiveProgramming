...
BigInteger add(const BigInteger& a, const BigInteger& b, BigInteger& prev_op) {
    BigInteger t = a + b - prev_op;
    prev_op = t;
    return t;
}

BigInteger subtract(const BigInteger& a, const BigInteger& b, BigInteger& prev_op) {
    BigInteger t = (a - b) * prev_op;
    prev_op = t;
    return t;
}

BigInteger multiply(const BigInteger& a, const BigInteger& b, BigInteger& prev_op) {
    BigInteger t = (a * b) * (a * b);
    prev_op = t;
    return t;
}

BigInteger division(const BigInteger& a, const BigInteger& b, BigInteger& prev_op) {
    BigInteger t;
    if (a % 2 == 0) {
        t = a / 2;
    } else {
...
int main() {
    BigInteger a, b;
    int T;
    BigInteger prev_op = 1;
    cin >> T;
    while (T--) {
        char op;
        cin >> a >> op >> b;
        if (op == '*') cout << multiply(a, b, prev_op) << "\n";
        else if (op == '+') cout << add(a, b, prev_op) << "\n";
        else if (op == '-') cout << subtract(a, b, prev_op) << "\n";
        else if (op == '/') cout << division(a, b, prev_op) << "\n";
        else cout << "ERROR\n";
    }
